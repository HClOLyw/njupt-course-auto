...
> 🖱 **只想直接用？** 下载 [`dist/NJUPT课程助手.exe`](dist/NJUPT课程助手.exe) 双击运行即可
> （Windows 64 位，免安装、无需 Python）。完整使用说明见下方
> [「直接使用（下载已打包的 EXE）」](#-直接使用下载已打包的-exe推荐)。
...
| 来源 | 位置 |
|------|------|
| GitHub 仓库（推荐） | 打开 <https://github.com/HClOLyw/njupt-course-auto> → 进入 **`dist/`** 目录 → 点击 `NJUPT课程助手.exe` → 右上角 **Download** 下载 |
| 直链下载 | `https://github.com/HClOLyw/njupt-course-auto/raw/main/dist/NJUPT课程助手.exe` |
| 本机已打包 | `D:\part time cx\njupt-gui\dist\NJUPT课程助手.exe` |

> 文件约 12 MB，**单文件绿色版**：不需要安装、不写注册表，删除即卸载。

### 2. 使用步骤

1. 把 `NJUPT课程助手.exe` 复制到一个**单独的文件夹**（如 `D:\NJUPT助手\`）。
   程序会在自己所在目录生成 `config.json` 保存配置，单独放便于备份与迁移。
2. **双击运行**。若弹出 Windows SmartScreen"已保护你的电脑"提示：
   点 **更多信息** → **仍要运行**（程序未购买代码签名证书，属正常现象）。
3. 进入 **⚙ 参数设置** → 先选 **教育类型**（实验室安全教育 / 新生教育）→
   粘贴对应平台的 **Access-Token** → 点 **保存配置**。
   （设置页只显示所选教育类型的字段，同一时刻只会看到一个 Access-Token 输入框）
4. 到对应功能页点击按钮：
   - **实验室安全教育**：开始刷实验室课程 / 查询课程进度 / 自动考试
     （考试前需先「下载承诺书模板」→ 打印签字 → 「上传已签字承诺书」）
   - **新生教育**：自动刷课 / 进度查询 / 自动考试
5. 运行日志实时显示在页面下方（成功 / 警告 / 错误分色），底部状态栏显示"空闲 / 运行中"。

### 3. 获取 Access-Token

| 平台 | 登录地址 | 获取方式 |
|------|---------|---------|
| 在线课堂（新生教育） | <https://study.njupt.edu.cn> | F12 → Application → Local Storage → 复制 `Access-Token` 的值（约 7 天过期） |
| 实验室平台（安全教育） | <http://10.22.192.38:9092>（校园内网） | 同上：复制 Local Storage 里的 `Access-Token` |

### 4. 多账号 / 换账号

- 换账号只需在 **参数设置** 里把该平台的 Token 换成新账号的并保存；
  实验室模式下课程与考试会**自动按新账号重新获取**，无需改动任何 ID。
- 两个平台的 Token 分开保存，来回切换教育类型**不会互相覆盖**。

### 5. 注意事项

- 仅支持 **Windows 64 位**（Win10 / Win11 均可），无需 Python 环境。
- 首次运行请先点一次 **保存配置**，`config.json` 才会生成在 exe 同目录。
- `config.json` 内含登录凭证，**请勿分享该文件或含 Token 的截图**。
- 若杀毒软件误报，添加信任即可；想自行编译见下方「打包成独立 EXE」。
...
3. 打包完成后，生成的程序位于 **`dist\NJUPT课程助手.exe`**（可直接分发使用）。

> 提示：打包前请先关闭正在运行的 `NJUPT课程助手.exe`，否则会报"拒绝访问"（文件被占用）。
> 程序以 exe 同目录下的 `config.json` 读写配置，首次运行请先保存一次配置。
> 仓库 `dist/` 中已附带打包好的 exe，可直接下载使用，无需自行编译（见上文「直接使用」）。
